<template>
  <section class="self-endmt-20 w-full max-w-full max-md:mt-10 max-md:max-w-full px-4">
    <div class="flex gap-5 justify-between max-md:flex-col">
      <div class="flex flex-col md:w-[32%] max-md:ml-0 max-md:w-screen items-center">
        <img loading="lazy"
          src="https://cdn.builder.io/api/v1/image/assets/c49e2462ddbf40d0944f306255d5e3e6/3e6706bff0e5f2f2fc01c2590465134e29e634c237102dd2e59d72afbd0ee055?apiKey=c49e2462ddbf40d0944f306255d5e3e6&"
          alt="Mobile payment illustration"
          class="object-contain grow w-4/5 aspect-[0.87] max-md:mt-10 max-md:max-w-full" />
      </div>
      <div class="flex flex-col max-lg:pt-10 w-3/5 max-md:w-full">
        <div
          class="flex flex-col items-start self-stretch my-auto text-center text-black max-md:mt-10 max-md:max-w-full">
          <h1 class="text-3xl font-bold max-md:max-w-full max-lg:text-2xl">
            Discover a new way to pay for your daily retail goods & services
          </h1>
          <p class="mt-6 text-xl font-medium max-md:max-w-full">
            Stop worrying about small change and start using Xpenspay today
          </p>
        </div>
      </div>
      <div class="flex flex-col w-1/4 max-md:w-full ">
        <a href="https://play.google.com/store/apps/details?id=com.simplex.xpenspay"
          class="flex flex-col self-stretch my-auto text-lg font-medium items-center text-center text-black max-md:mt-2">
          <img loading="lazy"
            src="../assets/img/undefined.svg"
            alt="QR code to download app" class="object-contain w-3/5 rounded-lg aspect-square" />
          <p class="self-center my-2.5">Scan to download the app.</p>
        </a>
      </div>
    </div>
    <!-- src="https://cdn.builder.io/api/v1/image/assets/c49e2462ddbf40d0944f306255d5e3e6/51c1493f4603db66d58732894835b9b7f1ff9aca7a1bb2479a615eca0bbb5692?apiKey=c49e2462ddbf40d0944f306255d5e3e6&" -->
  </section>
  <div
    class="flex flex-col items-center px-5 py-5 w-full md:text-2xl font-medium text-white bg-cyan-500 max-md:px-8 max-md:max-w-full">
    <div class="flex flex-wrap gap-3 justify-center items-end ml-5 max-md:max-w-full">
      <p class="max-md:max-w-full">
        A Product of Direct Distribution Crafts Limited. Powered by
      </p>
      <div class="flex items-center justify-center min-w-[240px]">
        <img loading="lazy"
          src="https://cdn.builder.io/api/v1/image/assets/c49e2462ddbf40d0944f306255d5e3e6/57ebeb0401582b719bc27f3540e484cd3d01630da939e672432cb3e5b8a2df13?apiKey=c49e2462ddbf40d0944f306255d5e3e6&"
          alt="VFD Microfinance Bank logo" class="object-contain shrink-0 aspect-[1.18] w-[39px]" />
        <span>VFD Microfinace Bank</span>
      </div>
    </div>
  </div>
</template>
